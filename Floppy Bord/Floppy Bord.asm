...
blocks
	.byte 0					;Ablage f�r die Anzahl der Bildschirme die gescrollt werden
clocks
	.byte 0					;Finescroll, Anzahl der Color Clocks um die geschoben werden soll

xr	.byte 0					;Sicherer Platz f�r Register
yr	.byte 0
a	.byte 0
...
scroll
	...
	ldx xr					;wait <>0=> zur�ck, nicht scrollen!
	ldy yr					;Register zur�ck schreiben
	jmp $e462				;VBI verlassen
s11
	lda #1					;wait zur�cksetzen
	sta wait	
	lda clocks				;Feinscroll?
	beq hard				;Nein! => Hardscroll
	dec clocks				;Feinscroll	
	lda clocks				;clocks=3,2,1,0 => beim Herunterz�hlen werden die Zeichen 		
	sta $d404				;nach links verschoben, pixelweise
	ldx xr					;Register zur�ck schreiben
	ldy yr					
	jmp $e45f				;VBI verlassen
hard	
	lda #3					;Finescroll Register zur�cksetzen					
	sta $d404
	sta clocks
...
s1	
	...
	;
	; Scrollbereich zur�cksetzen auf Anfang
	; Analog zur Routine in "Screeninit"
	;
	
	lda #190				;ja!
	sta blocks				;Anzahl der zu scrollenden Bildschirme zur�cksetzen
